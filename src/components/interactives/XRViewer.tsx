...
export default function XRViewer({ src, alt }: XRViewerProps) {
  ...
  if (!isEngineReady) {
    return (
      <div className="flex flex-col items-center justify-center h-full w-full rounded-2xl bg-black/20 backdrop-blur-sm border border-white/10">
        ...
        <span className="text-stone-400 text-xs font-mono uppercase tracking-widest">Booting WebXR Engine...</span>
      </div>
    );
  }
  ...
  return (
    <div className="relative w-full h-full flex items-center justify-center">
      {/* @ts-expect-error - React compiler bypass for custom web components */}
      <model-viewer
        src={src}
        alt={alt}
        camera-controls
