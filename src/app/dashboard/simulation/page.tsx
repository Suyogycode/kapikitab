...
export default function SimulationPage() {
  ...
  if (isLoading) return <div className="flex-1 w-full flex items-center justify-center min-h-screen"><Loader2 className="animate-spin text-stone-500" size={32} /></div>;
  if (simulations.length === 0 || !activeSimId) return <div className="p-8 text-stone-500 text-center">No 3D virtual labs available yet.</div>;
...
{/* Right Side: LIVE WEBXR MODEL VIEWER */}
      ...
      <div className={`mt-10 transition-opacity duration-500 ${isFullscreen ? 'opacity-0 hidden' : 'opacity-100'}`}>
        ...
        <div className="flex space-x-5 overflow-x-auto no-scrollbar pb-6 pl-2 -ml-2">
          {inactiveSims.map((sim) => (
            ...
            >
              ...
              <p className="text-sm text-stone-500 font-light leading-relaxed truncate">{sim.subtitle}</p>
            </motion.button>
          ))}
        </div>
      </div>
