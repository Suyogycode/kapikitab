...
const handleAssetSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
  ...
  // Note: Ensure your HTML file input field has name="file" to match this
  ...
  try {
    ...
    // 1. If a file exists in the form, initiate the secure R2 upload pipeline
    if (file && file.size > 0) {
      // Build proper payload structure for your routing logic
      const uploadFormData = new FormData();
      uploadFormData.append("file", file);
      uploadFormData.append("type", "image"); // Directs route to Cloudflare R2 branch

      // Fetch the pre-signed URL and public path mappings from Next.js
      ...
      if (!response.ok) {
        throw new Error('Failed to obtain a secure upload authorization token.');
      }

      const data = await response.json();

      if (data.isPresigned) {
        // Stream the raw binary file directly from the browser to Cloudflare R2
        const uploadRes = await fetch(data.uploadUrl, {
          method: 'PUT',
          body: file, // Fixed: point to 'file' instead of undefined 'fileObject'
          headers: {
        ...
        if (!uploadRes.ok) {
          throw new Error("Direct-to-R2 upload streaming failed.");
        }
      }

      // Fixed: 'data.url' contains the public distribution path generated by your API route
      uploadedUrl = data.url;
      console.log("File successfully pushed to R2:", uploadedUrl);
    }

// ... [Previous R2 Upload Code Stays the Same] ...

    // 2. Map Frontend Types to Backend Schema Enums
    ...
    // Fallback to diagram if panel type is missing
    const schemaCompliantType = panel.type ? typeMapping[panel.type] : 'diagram';

    // Generate a Semantic Asset ID (e.g., ast-ch-c11-math-01-u01-1784277...)
    const semanticAssetId = `ast-${chapterId}-${panel.unitId}-${Date.now()}`;

    // Build the payload EXACTLY as the Mongoose Schema demands
    contentPayload = {
      assetId: semanticAssetId,
      chapterId: chapterId,
      unitId: panel.unitId,
      order: 1, // Defaulting to 1, or you can dynamically calculate max order based on existing assets
      type: schemaCompliantType, 
      title: title,
      content: uploadedUrl, // The Cloudflare R2 URL must go here
    };

    // 3. Make the save network call to your Asset route
    ...
    if (!dbResponse.ok) {
      // You can also parse the response to see the exact MongoDB error in your console
      const errData = await dbResponse.json();
      console.error("Backend DB Error:", errData);
      throw new Error('Database transaction validation failed.');
    }

    // Reset panel status on successful submission
    setPanel({ isOpen: false, type: null, unitId: null });
    await fetchDataSummary();
    
  } catch (error) {
    console.error('Pipeline Upload Process Interrupted:', error);
